...
const extension = {
    eezFlowExtensionInit: (eezStudio: IEezStudio) => {
        ...
        const POSTGRESS_ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 34 38">
            <path
                fillOpacity=".702"
        ...
        function expandSqlParams(flowState: IFlowState, sql: string) {
            let expandedSql = sql;

            parse(sql).inputs.forEach(expression => {
                const value = flowState.evalExpression(expression);
        ...
        registerActionComponent({
            ...
            execute: async (flowState, ...[connection, sql]) => {
                const pgConnection = flowState.evalExpression(connection);
                if (!pgConnection) {
                    throw `connection "${connection}" not found`;
                }

                if (!(pgConnection instanceof PostgreSQLConnection)) {
                    throw `"${connection}" is not PostgreSQLConnection`;
                }

                const client = new pg.Client(
                    Object.assign({}, pgConnection.constructorParams, {
                        ssl: pgConnection.constructorParams.ssl
                            ? {
                ...
                try {
                    ...
                    pgConnection.error = undefined;
                } catch (err) {
                    pgConnection.error = err.toString();
                    throw err;
                }

                const expandedSql = expandSqlParams(flowState, sql);

                flowState.log("debug", `PostgreSQL: ${expandedSql}`);

                const res = await client.query(expandedSql);

                flowState.log("debug", `PostgreSQL result: ${res}`);

                await client.end();

                flowState.propagateValue("result", res);
